...
function encodeToRot13(tr) {
  ...
  return tr.replace(
    /[a-z]/gi,
    (letter) => originAlphabet[originAlphabet.indexOf(letter) + 13]
  );
}
...
function getCardId(value) {
  ...
  const suitsOfCards = ['♣', '♦', '♥', '♠'];
  return (
    numbersOfCards.indexOf(value.slice(0, -1)) +
    suitsOfCards.indexOf(value.slice(-1)) * numbersOfCards.length
  );
}
